...
final class ImageDownloadManager {
    ...
    func downloadImage(_ flickrPhoto: String, indexPath: IndexPath?, size: String = "m", handler: @escaping ImageDownloadHandler) {
        self.completionHandler = handler
        
        if let cachedImage = imageCache.object(forKey: flickrPhoto as NSString) {
            /* check for the cached image for url, if YES then return the cached image */
            print("Return cached Image for \(flickrPhoto)")
            self.completionHandler?(cachedImage, flickrPhoto, indexPath, nil)
        } else {
             /* check if there is a download task that is currently downloading the same image. */
            if let operations = (imageDownloadQueue.operations as? [PGOperation])?.filter({$0.imageUrl.absoluteString == flickrPhoto.absoluteString && $0.isFinished == false && $0.isExecuting == true }), let operation = operations.first {
    ...
    func slowDownImageDownloadTaskfor (_ flickrPhoto: String) {
